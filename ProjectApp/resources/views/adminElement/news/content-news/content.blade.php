<div class="card">
    <div class="card-header bg-header">
        <h4 class="text-center text-light">ตั้งค่าข่าว</h4>
    </div>
    <div class="card-body">
        <button type="button" class="btn float-right m-4" data-toggle="modal" data-target="#exampleModal">
            <i class="fa fa-plus-circle" aria-hidden="true" style="color:#A9CEE0;font-size:30px"></i>
        </button>

        <div class="table-responsive">
            <table class="table txt-greenblue" style="border:2px solid #639cb4">
                <thead class="table-info" style="border-bottom-color:#639cb4">
                    <tr class="text-center ">
                        <th class="border-left">ลำดับ</th>
                        <th>ที่อยู่เว็บ</th>
                        <th>หน้าปก</th>
                        <th class="border-right"></th>
                    </tr>
                </thead>
                <tbody style="border-top-color:#639cb4">
                    <tr class="text-center  ">
                        <td class="align-middle border-left-0">1</td>
                        <td class="align-middle">https://en.wikipedia.org/wiki/Spanning_Tree_Protocol</td>
                        <td class="align-middle"><img class="rounded mx-auto d-block" width="280px" height="180px"
                                src="data:image/jpeg;base64,"
                                alt=""></td>
                        <td class="align-middle border-right-0">
                            <button class="btn border-danger">
                                <i class="fa fa-trash" aria-hidden="true" style="color:gray"></i>
                            </button>
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>


<!-- Modal -->
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content container">
            <div class="modal-header" style="border-bottom-color:#639cb4">
                <h5 class="modal-title txt-greenblue" id="exampleModalLabel">เพิ่มข่าวใหม่</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <div class="row was-validated container ">
                    <div class="col-md-12">
                        <label for="validatedCustomFile" class="txt-greenblue">เลือกไฟล์</label>
                        <div class="custom-file">
                            <input type="file" class="custom-file-input boder-greenBlue" style="background:#8FACB7" id="validatedCustomFile" required>
                            <label class="custom-file-label"  for="validatedCustomFile">Choose file</label>
                            <div class="invalid-feedback">กรุณาเลือกไฟล์</div>
                        </div><br>
                        <div class="form-group">
                            <label class="txt-greenblue" for="formGroupExampleInput">ที่อยู่เว็บไซต์</label>
                            <input type="text" class="form-control boder-greenBlue" id="formGroupExampleInput" required>
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer w-100" style="border-top-color:#639cb4">
                <button type="button" class="btn btn-primary m-auto text-light" style="background:#56A1C0">เพิ่ม</button>
            </div>
        </div>
    </div>
</div>
