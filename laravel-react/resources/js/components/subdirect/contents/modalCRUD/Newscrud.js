...
export function Newsedit(props){
    ...
    return(
        <div key={props.id} className="mr-1">
            <Button name="modalEdit" variant="warning" size="sm" onClick={() => setEditModal(true)} > แก้ไข</Button>{' || '}
            <Modal
                show={editModal}
...
export function Newsdelete(props){
    ...
    return(
        <div key={props.id}>
            <Button name="modalDelete" variant="danger" size="sm" onClick={() => setDeleteModal(true)} >ลบ</Button>
            <Modal
                show={deleteModal}
